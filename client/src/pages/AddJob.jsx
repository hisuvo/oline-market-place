...
const AddJob = () => {
  ...
  const handleJobPost = (event) => {
    ...
    // One: post FormData in server
    // axios
    //   .post(`${import.meta.env.VITE_API_URL}/jobs-add`, formData)
    //   .then((res) => {
    ...
    // Two : another way by using try___catch
    try {
      axios.post(`${import.meta.env.VITE_API_URL}/jobs-add`, formData);
      toast.success("Job post done");
      navigate("/my-posted-jobs");
    } catch (error) {
      toast.error(error.message);
